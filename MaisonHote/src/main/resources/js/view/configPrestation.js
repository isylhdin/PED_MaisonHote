window.EditPrestationView = Backbone.View.extend({
	...
	initialize: function () {
		$(this.el).append('<header class="jumbotron subhead" id="overview" '+
			...
			'<h1>Edition des prestations</h1></div></header>');
		this.render();
	},
	...
	render: function (){
		...
		window.prestations.localStorage =
	 ...
	 */
	footpage : function(){
		this.$('#prestation').append('<div class="row" id="add">' +
			'<button class="btn btn-success"><i class="icon-plus icon-white">' +
			'</i> Ajouter</button></div>');
		$(this.el).append(
			'<hr><div class="row" align="center"><button type="submit" ' +
			'id="submit" class="btn">Enregistrer</button></div>');
		$(this.el).append(
			'<div id="waitingResult" style="visibility:hidden" ' +
	...
	onAdd: function(event) {
		...
		if (parseInt(nbPrest) == 1) {
			...
			$('#rowPrestation' + previous).after(
				this.template(prestation.toJSON()));
		}	
	...
	onSubmit: function(event) {
		...
		prestations.each(function(Prestation){
			...
			Prestation.save({
				'title': title, 'price': price, 'comment': comment
			}, {
				success: function(model, response, options) {
		...
		var obj = JSON.parse(localStorage.getItem(
				'fichier-backbone-house_config_prestations.json'));

		updateFile(obj.idFichier, JSON.stringify(prestations.toJSON()),
				function(reponse) {	
			...
			if (!reponse.error && nbPrestationsSauveesDansCache == nbPrest) {
				...
				console.log(
					'ERROR § ' + nbPrestationsSauveesDansCache +
					' prestations sur ' + nbPrest +
	 ...
	 */
	modal: function(id) {
		$('#rowPrestation' + id).append(
			_.template(tpl.get('DeleteModalView')));
		$('h3').text('Suppression');
	 ...
	 * supprimée pour avoir un cache cohérent avec l'UI
	 * et renumérote les chambres suivantes
	 */
	deleteFromCache : function(){
		console.log('id de la prestation supprimée = ' + id +
			' et nbPrestaion = ' + nbPrest);
		if (id <= nbPrest) {
