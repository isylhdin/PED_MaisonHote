window.SelectChambreView = Backbone.View.extend({
	...
	// Quand on clique sur un numéro de la liste on construit l'ui (avec les
	// id pas encore définis).
	// Chaque "input" doit avoir un id modifié dynamiquement
	constructForm: function(nbChambre) {
		// pas top : quand on clique sur un autre item de la liste :
		// la précédente collection (avec ses éléments) ne sont pas supprimés
		// => memory leak
		chambres = new Chambres();

		$('#maison').empty();
		for (var i = 1; i <= nbChambre; i++){
			// crée une chambre juste pour set les id comme il faut dans
			// le code html
			...
			$('#maison').append(
				this.template(window['chambre' + i].toJSON()));	
	...
	constructFormService: function () {
		...
		$('#prestation').append(
			this.template(window['prestation' + nbPrest].toJSON()));
	},
	...
	saveDataRoom: function() {
		var price, litSimple, litDouble, litJumeau,
			tele, internet, baignoire, douche;

		for (var i = 1; i <= nbChambre; i++) {
			price = $('#inputPrice' + i).val();

			litSimple = $('#litSimple'+i).val();
			litDouble = $('#litDouble'+i).val();
			litJumeau = $('#litJumeau'+i).val();

			tele = $('input[name=tele' + i + ']').is(':checked');
			internet = $('input[name=internet' + i + ']').is(':checked');
			baignoire = $('input[name=baignoire' + i + ']').is(':checked');
			douche = $('input[name=douche' + i + ']').is(':checked');

			// set les chambres dans la collection et les sauvegarde une
			// par une dans le cache
			window['chambre' + i].save({
				'prixParJour': price, 'tele':tele, 'internet':internet,
				'baignoire': baignoire, 'douche': douche,
				'litSimple': litSimple, 'litDouble': litDouble,
				'litJumeau': litJumeau
			}); 
		}	
	},
	...
	saveDataService: function(){
		var titleP, priceP, commentP;

		for (var i = 1; i <= nbPrest; i++) {
			titleP = $('#title' + i).val();
			priceP = $('#price' + i).val();
			commentP = $('#comment' + i).val();

			console.log(window['prestation' + i]);
			// set les prestations dans la collection et les sauvegarde une
			// par une dans le cache
			window['prestation' + i].save({
				'title': titleP,'price': priceP, 'comment': commentP
			}); 
		}
	},
	...
	createFileChambre: function(){	
		createNewFile('house_config_chambres.json', function(reponse) {
			...
			// on conserve l'id du fichier dans le cache pour pouvoir utiliser
			// le web service d'update dessus (a besoin de son id)
			var houseConfig = new FichierConfig({
				'id': reponse.title, 'idFichier': idHouseConfig
			});
			houseConfig.save();

			updateFile(reponse.id, JSON.stringify(chambres.toJSON()),
					function(reponse) {
				console.log(reponse);
			});
	...
	createFileService: function(){
		createNewFile('house_config_prestations.json', function(reponse) {	
			...
			if (typeof(prestations) !== 'undefined') {
				updateFile(reponse.id, JSON.stringify(prestations.toJSON()),
						function(reponse) {	
					console.log(reponse);
				});
			}
	...
	createFileClient: function() {
		createNewFile('customers.json', function(reponse) {	
			...
			var clientFile = new FichierConfig({
				'id': reponse.title, 'idFichier': idClient
			});
			clientFile.save();
	...
	createFileResa: function() {
		createNewFile('resa.json', function(reponse) {	
			...
			var resaFile = new FichierConfig({
				'id': reponse.title, 'idFichier': idResa
			});
			resaFile.save();
	...
	createFileOrderedPresta: function() {
		createNewFile('ordered_prestas.json', function(reponse) {	
			...
			var prestaResaFile = new FichierConfig({
				'id': reponse.title, 'idFichier': idPrestaResa
			});
			prestaResaFile.save();
		});
	},
	...
	modal: function() {
		...
		$('h4').text('C\'est votre première utilisation, ' +
			'vous allez configurer vos chambres !');
		$('#modal').modal(); 		
	},
});
